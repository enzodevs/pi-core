...
On genuine session shutdown (`quit`, `new`, `resume`, or `fork`), Pi Core idempotently enqueues the finalized session path and content hash without copying its transcript into canonical memory; extension reloads are ignored because their session file remains mutable. After shutdown enqueues the finalized session, one detached `openai-codex/gpt-5.6-terra` process at medium reasoning consolidates up to four oldest pending jobs through validated `agent-memory` candidates. A private lock prevents concurrent consolidators, the run is capped at 15 minutes, and a dedicated system prompt requires all memory writes to pass through the validated CLI. Project candidates may apply after deterministic validation; global candidates require explicit human approval, contradictions must use supersession, and automatic hard deletion is unavailable. Shutdown reconciliation terminalizes analyzed jobs whose candidates are already terminal. `agent-memory` remains responsible for Markdown truth, SQLite FTS/vector indexes, temporal metadata, candidates, provenance, stale-source checks, maintenance metrics, and atomic application.

Pi Core initializes an empty project root on first use. If `agent-memory` is missing or retrieval fails, Pi continues without injected context. `/memory-status` shows bounded queue metrics. Pending global proposals appear in the footer and can only be confirmed interactively with `/memory-approve candidate-<id>`; manual maintenance remains available through the portable agent-memory skill and CLI.
